...
* **Deterministic Testing:** Ensure `python -B tests/run_all.py` reports all suites green with zero `[FAIL]` lines before handoff (count is dynamic, currently 70/70 — read it from `tests/run_all.py` output, never match a hardcoded number).
