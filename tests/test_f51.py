...
blk = prompts._recent_fact_lines(cap=300, db=db)
...
c.commit(); c.close()
blk2 = prompts._recent_fact_lines(cap=5, db=db2)
...
check("no facts -> '(none yet)'", prompts._recent_fact_lines(db=make_db(0, tmp / "empty.db")),
      "(none yet)")
...
if live.exists():
    ...
    print(f"  live window holds {n} facts; shipped cap is {prompts.FACT_LEDGER_CAP}")
    check("shipped cap exceeds the live window", n < prompts.FACT_LEDGER_CAP, True)
    check("cap did not regress below the old 120", prompts.FACT_LEDGER_CAP >= 120, True)
    check("live block is not truncated today",
          "TRUNCATED BY THE HARNESS" in prompts._recent_fact_lines(), False)
else:
