...
**AS-BUILT DEVIATION (2026-07-31) — the DEFER verdict above was OVERRIDDEN by the operator, deliberately.** `orchestrator/simulate.py` (567 lines, stdlib only) was built in a parallel session and exists on disk **UNCOMMITTED, by explicit operator instruction** ("let's not worry about committing for a while"). It implements three prediction domains — task outcome, video engagement, skill-promotion safety — plus the closed loop this section's revisit trigger describes: predict → record → measure error → feed the error history back.

**Operator's stated reasoning, which is the actual decision record:** *"an AGI would likely predict human behavior more accurately, so that was my reasoning."* That reframes simulation from a cost-optimization (this section's framing — simulate only when acting is expensive) into a **capability claim**: predicting audience response IS the thing being built, not a way to avoid paying for actions. Under that framing the M3 gate does not apply, because the trigger was never the point. Do not delete or "correct" this module as out-of-spec — it is authorized.

**What is NOT yet true of it, measured 2026-07-31 and stated so the numbers are not trusted prematurely:**
- **Video-engagement accuracy (reported 12.8% / 14.7% error) is not evidence.** The recorded "actual" outcomes were 180,000 and 95,000 views; **no video in the 17-record training set has either figure** (nearest: 186,000 and 94,000), and both predictions were for hypothetical videos never published. The errors measure agreement with hand-entered numbers, not with reality.
- **Task-outcome accuracy (reported 0.4%) is real arithmetic on a near-tautology.** It predicted the median cost of a mission and scored against another run of that same scripted mission (predicted 4,518,017; task #24 actually 4,501,536), and was labelled W32 while validated against a completed W31 row. Low variance in a repeated job, not foresight.
- **Skill-safety training data is factually wrong.** `predict_skill_safety()` hardcodes `"regressed": False  # No regressions observed yet` for every known skill. A rollback *has* occurred — commit `c7b5721`, on a mission-001 skill — so the single real negative example in the project's history is invisible to the model, on the very mission it scored "low risk".
- Not wired into `batch_runner.py`, `promote.py`, or `scorecard.py`; inert unless invoked manually.

**The architecture is sound and the validation is not.** Both halves are true at once, and the second is cheap to fix: record outcomes only from real published videos and real completed tasks. The weekly Vaibhav tracker cron (`e6e05b1d2e8a`, Mondays 09:00) is the mechanism that will supply genuine outcome data over time. Until it does, treat every accuracy figure this module reports as unvalidated.
